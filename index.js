...
function createForceMatrixTable() {
    ...
    colors.forEach((color, idx1) => {
        ...
        colors.forEach((_color2, idx2) => {
            ...
            input.onchange = (e) => {
                forceMatrix[idx1][idx2] = parseFloat(e.target.value)
            }
...
const FORCE_RANGE_SQUARED = FORCE_RANGE * FORCE_RANGE
function applyPointPointForces(deltaT) {
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      for (let j = i+1; j < points.length; j++) {
            ...
            const deltaX = p2.pos[0] - x0
            const deltaY = p2.pos[1] - y0

            const distSquared = deltaX * deltaX  + deltaY * deltaY
            if (distSquared > FORCE_RANGE_SQUARED) continue
